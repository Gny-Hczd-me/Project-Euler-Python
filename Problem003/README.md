...
​Reduction: When a prime factor is found, it is stored in the max variable, and the target number a is divided by this factor to reduce the search space.
​Result: Once the loop completes, the last stored max value is printed as the largest prime factor.